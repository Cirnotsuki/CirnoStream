...
export default function formatAttrs(
    ...
    if (typeof EventData.handler !== 'function' && !EventData.eventList.length) {
        throw new Error('Property<handler or eventList> of StreamEvent should atleast includes an excutionable Function.');
    }

    // format EventList
    EventData.eventList = EventData.eventList.map(fn => (typeof fn === 'function' ? fn : () => { }));
    ...
    if (!EventData.handler && EventData.remainTimes !== 0 && EventData.eventList.length > EventData.remainTimes) {
        EventData.remainTimes = EventData.eventList.length;
    }
    ...
    // when Attribute repeat is 0, StreamEvent will continually execute according to Attribute: delay
    EventData.isLoopEvent = EventData.repeat === 0;

    // if Argument _isLoopEvent is Boolean, then set Attribute: repeat follow the rules.
    // when Attribute: isLoopEvent is true, StreamEvent will continually execute,
