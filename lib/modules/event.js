...
class StreamEvent {
  ...
  get close() {
      ...
      // if Event has been closed and trigger the onclose function.
          if (typeof event.onclose === 'function') {
              event.onclose(CallbackEvent(event.data, 'closed'));
          }
          event.onclose = null;
  ...
  get decrease() {
      ...
      return function EventDecrease(argEvent) {
          if (argEvent !== event) throw new Error('EventDecrease should not be excuted manualy.');
  ...
  get refresh() {
      ...
      return function EvnetRefresh(argEvent) {
          if (argEvent !== event) throw new Error('EvnetRefresh should not be excuted manualy.');
