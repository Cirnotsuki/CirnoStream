...
class StreamEvent {
  ...
  constructor(stream, data) {
      this.#stream = stream;
      this.#data = data;
      // it will be executed when StreamEvent has been created.
      this.created = data.created;
      // it will be executed when StreamEvent has been removed.
      this.onclose = data.onclose;
      // function in handler will be executed when StreamEvent on its trigger-time that depends on Argument: delay.
      this.handler = data.handler;
      // function in eventList will be executed one by one, and its priority is greater than handler.
  ...
  get decrease() {
      ...
      return function EventDecrease(argEvent) {
          if (argEvent !== event) throw new Error('EventDecrease should not be executed manualy.');

          event.#data.remainTimes -= 1;

          // remove event when it is not a continually event or remain execute time go to zero.
  ...
  get refresh() {
      ...
      return function EvnetRefresh(argEvent) {
          if (argEvent !== event) throw new Error('EvnetRefresh should not be executed manualy.');
